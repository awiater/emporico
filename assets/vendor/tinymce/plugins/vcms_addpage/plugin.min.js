...
tinymce.PluginManager.add('vcms_addpage', function(editor) {

	function vcms_addpage_onMessage(event){
		if(editor.settings.external_filemanager_path.toLowerCase().indexOf(event.origin.toLowerCase()) === 0){
			if(event.data.sender === 'vcms_addpage'){
				...
				if(window.removeEventListener){
					window.removeEventListener('message', vcms_addpage_onMessage, false);
				} else {
					window.detachEvent('onmessage', vcms_addpage_onMessage);
				}
			}
		}
	}
	...
	function openmanager() {
		...
		if (typeof editor.settings.filemanager_crossdomain !== "undefined" && editor.settings.filemanager_crossdomain) {
			...
			if(window.addEventListener){
				window.addEventListener('message', vcms_addpage_onMessage, false);
			} else {
				window.attachEvent('onmessage', vcms_addpage_onMessage);
			}
		}
	...
	if (tinymce.majorVersion < 5) {
		...
		editor.addMenuItem('vcms_addpage', {
			icon: 'browse',
			text: 'Insert file',
			shortcut: 'Ctrl+E',
			onClick: openmanager,
		...
		editor.ui.registry.addButton('vcms_addpage', {
			icon: 'browse',
			tooltip: 'Insert file',
			shortcut: 'Ctrl+E',
		...
		editor.ui.registry.addMenuItem('vcms_addpage', {
			icon: 'browse',
			text: 'Insert file',
			shortcut: 'Ctrl+E',
			onAction: openmanager,
